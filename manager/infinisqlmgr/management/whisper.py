# Copyright 2013 Christopher Nelson (changes to run under Python 3.3)
# Copyright 2008 Orbitz WorldWide (original version)
#
...
aggregationMethodToType = dict([[v,k] for k,v in list(aggregationTypeToMethod.items())])
aggregationMethods = list(aggregationTypeToMethod.values())
...
def __propagate(fh,header,timestamp,higher,lower):
  ...
  fh.seek(int(higherFirstOffset))

  if higherFirstOffset < higherLastOffset: #we don't wrap the archive
    seriesString = fh.read(int(higherLastOffset - higherFirstOffset))
  else: #We do wrap the archive
    higherEnd = higher['offset'] + higher['size']
    seriesString = fh.read(int(higherEnd - higherFirstOffset))
    fh.seek(int(higher['offset']))
    seriesString += fh.read(int(higherLastOffset - higher['offset']))

  #Now we unpack the series data we just read
  byteOrder,pointTypes = pointFormat[0],pointFormat[1:]
  points = int(len(seriesString) / pointSize)
  ...
  for i in range(0,len(unpackedSeries),2):
    ...
    if pointTime == currentInterval:
      neighborValues[int(i/2)] = unpackedSeries[i+1]
...
def __archive_update_many(fh,header,archive,points):
  ...
  alignedPoints = list(dict(alignedPoints).items()) # Take the last val of duplicates
...
def file_fetch(fh, fromTime, untilTime):
  ...
  fh.seek(int(fromOffset))
  if fromOffset < untilOffset: #If we don't wrap around the archive
    seriesString = fh.read(int(untilOffset - fromOffset))
  else: #We do wrap around the archive, so we need two reads
    archiveEnd = archive['offset'] + archive['size']
    seriesString = fh.read(int(archiveEnd - fromOffset))
    fh.seek(archive['offset'])
    seriesString += fh.read(int(untilOffset - archive['offset']))
  ...
  for i in range(0,len(unpackedSeries),2):
    ...
    if pointTime == currentInterval:
      ...
      valueList[int(i/2)] = pointValue #in-place reassignment is faster than append()
    currentInterval += step
